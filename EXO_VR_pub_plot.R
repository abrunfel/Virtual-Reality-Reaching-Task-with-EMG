...
ggsave("rc.mc.ts.png", plot = p.ts, width = 16, height = 9, units = c("in"), dpi = 300)
#


# Fig 3 - Relative Contribution boxplots -------------------------------
...
ggsave("rc.boxplot.png", plot = last_plot(), width = 16, height = 9, units = c("in"), dpi = 300)
#


# Fig 4 - Muscle Contribution boxplots ------------------------------------
...
emg.data.ratio.mbb.bt = emg.data.ratio %>% select(-c(Trial, Day)) %>%
  group_by(Subject, Block, Condition, Load, Muscle, Target) %>%
  summarise_all(funs(mean(., na.rm = T)))

emg.data.ratio.mbb.bt = rename(emg.data.ratio, Hand = Condition)
...
ggsave("mc.boxplot.png", plot = last_plot(), width = 16, height = 9, units = c("in"), dpi = 300)
# # Regressions -------------------------------------------------------------
...
# 
# df.corr.bicep = rename(df.corr.bicep, Hand = Condition)
# df.corr.deltoid = rename(df.corr.deltoid, Hand = Condition)
# 
# p3 = ggplot(data = subset(df.corr.bicep, !Hand == "Baseline"), aes(x = emg.diff, y = rel.cont.diff))+
...
#        title = "Bicep")
# 
# p4 = ggplot(data = subset(df.corr.deltoid, !Hand == "Baseline"), aes(x = emg.diff, y = rel.cont.diff))+
#   geom_point(aes(color = Hand, shape = Load, size = 2))+
#   geom_smooth(method = 'lm')+
#   coord_cartesian(xlim = c(-21,21))+
#   theme_gray(base_size = 30)+
...
#        title = "Deltoid")
# 
# legend = gtable_filter(ggplotGrob(p3), "guide-box") # Extract Legend from p3
# ytext = textGrob("\u0394 Relative Contribution (%)", rot = 90, gp = gpar(fontsize = 26)) # create new axis text
# xtext = textGrob("\u0394 Muscle Contribution (%)", rot = 0, gp = gpar(fontsize = 26))
# p.reg = grid.arrange(p3 + theme(legend.position = "none", axis.title = element_blank()),
#                      p4 + theme(legend.position = "none", axis.title = element_blank()),
#                      nrow = 2, left = ytext, bottom = xtext, right = legend) # Set layout and reinsert axis text and legend
# 
# ggsave("rc.mc.regression.png", plot = p.reg, width = 16, height = 9, units = c("in"), dpi = 300)
#
