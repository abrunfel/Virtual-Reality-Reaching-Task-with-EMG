...
% Current version coded on 1/6/22
clear all; close all
%% Start Here!!!
...
% NOTE: this is mostly for plotting issues. Things like displacement won't
% be affected (they are relative measures).

% Do you want to invert the y, z axes?
% 1 = YES
% 0 = NO
invert = 1; % Default is 1
...
for k = 1:length(files)
    ...
    [DFexport, trialData, subid, cond] = vr_exo_kinR1(filename, invert);
    dispDF = dispCompare(trialData, DFexport, 7);
    if strcmp(computer, 'PCWIN64')
        ...
        dlmwrite('dispDF', dispDF, '-append', 'delimiter', ',', 'precision', '%.6f'); % Write dataframe to folder as comma delim file NOTE: this appends data, so you must delete unwanted (old) files before running this
        cd('C:\Users\Alex\Dropbox\Catholic U\VR_EXO\raw data\vr\Wrist weight and healthy control manuscript data');
    else
...
%%%%%%%%%%% looped in "Main Loop"
function [DFexport, trialData, subid, cond, block] = vr_exo_kinR1(file, invert)
% Data import
...
for i = 1:length(trialData)
    ...
    % Index of peak velocity (as midpoint integer value between LH and RH.
    % Used to calculate RC at peak velocity (see lab book pg. 111). These
    % indicies are passed into ExoVr_outcomeVars() where the actual
    % calculation takes place.
    trialData(i).indVP = round((indVPLH + indVPRH)/2);
end
...
%sdfhzdfbdf % Remove comment to create natural codebreak to prevent writing data to file.
%% Save data to file
...
function dispDF = dispCompare(trialData, DFexport, target)
% Reads in trialData and dataframe to generage dispacement vs. sample
% dataframes for each subject and condition. You must specify a target to
% export (this cuts down on programming for now...might want to expand this
% to include all data at a later point).
% Set up factors
nsamp = 100; % Resample length
subid = repelem(DFexport(1,1),nsamp)';
cond = repelem(DFexport(1,2),nsamp)';
block = repelem(DFexport(1,3),nsamp)';
handL = repelem(1,nsamp)';
handR = repelem(2,nsamp)';
sample = linspace(1,nsamp,nsamp)';
targetSpawn = DFexport(:,5);
% Resample
for i = 1:length(trialData)
    trials_lh = trialData(i).lhDisp'; % get lh data from onset-offset
    trials_rh = trialData(i).rhDisp'; % get lh data from onset-offset
    xq = linspace(1, length(trials_lh), nsamp)'; % Resample to this length array
...
lhDmean = zeros(nsamp,6);
rhDmean = zeros(nsamp,6);
lhDstd = zeros(nsamp,6);
rhDstd = zeros(nsamp,6);
for i = 5:10 % targets number from 5 to 10
    % Means
    lhDmean(:,i-4) = mean(lhD(:, find(targetSpawn == i)), 2);
    rhDmean(:,i-4) = mean(rhD(:, find(targetSpawn == i)), 2);
    % Standard deviations
    lhDstd(:,i-4) = std(lhD(:, find(targetSpawn == i)), 0, 2);
    rhDstd(:,i-4) = std(rhD(:, find(targetSpawn == i)), 0, 2);
end
%
% Subject & Block dataframe snippet
tempDFlh = [subid cond block handL sample lhDmean(:,target-4) lhDstd(:,target-4)];
tempDFrh = [subid cond block handR sample rhDmean(:,target-4) rhDstd(:,target-4)];
dispDF = [tempDFlh; tempDFrh];
end